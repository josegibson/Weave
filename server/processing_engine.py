...
import tempfile
import subprocess
import shutil  # Added to locate the LibreOffice executable
...
def process_presentation(pptx_path, excel_path, output_path, output_format):
    """Process a presentation by replacing placeholders with data from Excel or output PDF if requested."""
...
def main():
    ...
    # Add optional output format argument (pptx or pdf)
    process_parser.add_argument("-f", "--format", choices=["pptx", "pdf"], default="pptx", help="Output format: pptx or pdf")
    ...
    try:
        ...
        if args.command == "analyse":
            ...
            # Call processing and pass format
